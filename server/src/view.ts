...
class InternalFunctions {
    static async updateTuning(player: alt.Player, vehicle: alt.Vehicle) {
        ...
        for (let i = 0; i < veh2.tuning.mods.length; i++) {
            ...
            if (vehmods !== null) {
                alt.log('--- MODS | ID --- ' + vehmods.id);
                alt.log('--- MODS | VALUE --- ' + vehmods.value);
        ...
        //     if (vehmods !== null) {
        //         alt.log('--- STANCE | ID --- ' + vehmods.id);
        //         alt.log('--- STANCE | VALUE --- ' + vehmods.value);
        ...
        alt.log('--- STANCE --- ' +veh2.tuning.stance);


        if (veh2.tuning.stance[0].id) {
            vehicle.setStreamSyncedMeta('wheelModCamber', veh2.tuning.stance[0].value);
        }
        if (veh2.tuning.stance[1].id) {
            vehicle.setStreamSyncedMeta('wheelModHeight', veh2.tuning.stance[1].value);
        }
        if (veh2.tuning.stance[2].id) {
            vehicle.setStreamSyncedMeta('wheelModRimRadius', veh2.tuning.stance[2].value);
        }
        if (veh2.tuning.stance[3].id) {
            vehicle.setStreamSyncedMeta('wheelModTrackWidth', veh2.tuning.stance[3].value);
        }
        if (veh2.tuning.stance[4].id) {
            vehicle.setStreamSyncedMeta('wheelModTyreRadius', veh2.tuning.stance[4].value);
        }
        if (veh2.tuning.stance[5].id) {
            vehicle.setStreamSyncedMeta('wheelModTyreWidth', veh2.tuning.stance[5].value);
        }
...
export class TuningShopView {
    static init() {
        ...
        alt.onClient(Tuningmenu_Events.UPDATE_SERVER, InternalFunctions.updateTuning);
    ...
    static purchaseStance(
        ...
        alt.log('--- STANCE | MODIFICATION | ID --- '+name+' PURCHASE')
        alt.log('--- STANCE | MODIFICATION | VALUE --- '+value+' PURCHASE')
    ...
    static purchaseWheels(
        ...
        alt.log(' - WHEELS - MODIFICATION - FAIL - CHECK - TYPE - '+wheeltype)
        alt.log(' - WHEELS - MODIFICATION - FAIL - CHECK - ID - '+wheelid)
    ...
    static async payTuning(player: alt.Player, id: number, value: number, valuepl: number, valuewi: number, ids: number, values: number, wheel: string, idw: number, valuew: number){
