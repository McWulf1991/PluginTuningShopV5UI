...
async function emitUpdate() {
    ...
    alt.emitServer(Tuningmenu_Events.UPDATE_SERVER, vehicle);
}
...
async function payS(value: number, name: number) {
...
async function buy() {
    const id = await alt.LocalStorage.get('tune-pay-id-Tuning');
    const value = await alt.LocalStorage.get('tune-pay-value-Tuning');
    const valuepl = await alt.LocalStorage.get('tune-pay-value-Plate');
    const valuewi = await alt.LocalStorage.get('tune-pay-value-Window');
    const ids = await alt.LocalStorage.get('tune-pay-name-Stance');
    const values = await alt.LocalStorage.get('tune-pay-value-Stance');
    const wheel = await alt.LocalStorage.get('tune-pay-Wheel');
    const idw = await alt.LocalStorage.get('tune-pay-id-Wheel');
    const valuew = await alt.LocalStorage.get('tune-pay-value-Wheel');
    if(ids === 0) {
        camber(values)
    }
    if(ids === 1) {
